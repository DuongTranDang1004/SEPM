...
class AllAuthUserServiceTest {
    ...
    void testSendMessage_ShouldSucceedWhenUserIsParticipant() throws Exception {
        ...
        assertDoesNotThrow(() ->
                allAuthUserService.sendMessage(tenantAId, conversationId, request, null)
        );
    ...
    void testSendMessage_ShouldFailWhenTenantTriesToMessageThemselves() {
        // Arrange - Tenant A trying to send to conversation where they are the only participant
        Conversation conversation = Conversation.builder()
                .id(conversationId)
